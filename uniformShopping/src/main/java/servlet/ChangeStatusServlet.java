...
public class ChangeStatusServlet extends HttpServlet {
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			OrderItemDAO orderitemDao = new OrderItemDAO(); //OrderItemDAOオブジェクトの生成
			UserDAO userDao = new UserDAO(); //UserDAOオブジェクトの生成
			...
			ArrayList<OrderItem> mail_list = orderitemDao.selectByDate(buyer, purDate);
			User user = userDao.searchUser(buyer);

			if (payStatus.equals("2")) {
				send.PayMailText(mail_list, user);
			}
			if (deliStatus.equals("3")) {
				send.DeliMailText(mail_list, user);
			}
